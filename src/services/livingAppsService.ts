...
import type { Abteilungen, Stellen, Mitarbeiter, Abwesenheiten, Leistungsbeurteilungen, CreateAbteilungen, CreateStellen, CreateMitarbeiter, CreateAbwesenheiten, CreateLeistungsbeurteilungen } from '@/types/app';
...
export class LivingAppsService {
  // --- ABTEILUNGEN ---
  static async getAbteilungen(): Promise<Abteilungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ABTEILUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Abteilungen[];
    return enrichLookupFields(records, 'abteilungen');
  }
  static async getAbteilungenEntry(id: string): Promise<Abteilungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ABTEILUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Abteilungen;
    return enrichLookupFields([record], 'abteilungen')[0];
  }
  static async createAbteilungenEntry(fields: CreateAbteilungen) {
    return callApi('POST', `/apps/${APP_IDS.ABTEILUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'abteilungen') });
  }
  static async updateAbteilungenEntry(id: string, fields: Partial<CreateAbteilungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.ABTEILUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'abteilungen') });
  }
  static async deleteAbteilungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ABTEILUNGEN}/records/${id}`);
  }
  ...
  // --- ABWESENHEITEN ---
  static async getAbwesenheiten(): Promise<Abwesenheiten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ABWESENHEITEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Abwesenheiten[];
    return enrichLookupFields(records, 'abwesenheiten');
  }
  static async getAbwesenheitenEntry(id: string): Promise<Abwesenheiten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ABWESENHEITEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Abwesenheiten;
    return enrichLookupFields([record], 'abwesenheiten')[0];
  }
  static async createAbwesenheitenEntry(fields: CreateAbwesenheiten) {
    return callApi('POST', `/apps/${APP_IDS.ABWESENHEITEN}/records`, { fields: cleanFieldsForApi(fields as any, 'abwesenheiten') });
  }
  static async updateAbwesenheitenEntry(id: string, fields: Partial<CreateAbwesenheiten>) {
    return callApi('PATCH', `/apps/${APP_IDS.ABWESENHEITEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'abwesenheiten') });
  }
  static async deleteAbwesenheitenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ABWESENHEITEN}/records/${id}`);
  }

  // --- LEISTUNGSBEURTEILUNGEN ---
  static async getLeistungsbeurteilungen(): Promise<Leistungsbeurteilungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.LEISTUNGSBEURTEILUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Leistungsbeurteilungen[];
    return enrichLookupFields(records, 'leistungsbeurteilungen');
  }
  static async getLeistungsbeurteilungenEntry(id: string): Promise<Leistungsbeurteilungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.LEISTUNGSBEURTEILUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Leistungsbeurteilungen;
    return enrichLookupFields([record], 'leistungsbeurteilungen')[0];
  }
  static async createLeistungsbeurteilungenEntry(fields: CreateLeistungsbeurteilungen) {
    return callApi('POST', `/apps/${APP_IDS.LEISTUNGSBEURTEILUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'leistungsbeurteilungen') });
  }
  static async updateLeistungsbeurteilungenEntry(id: string, fields: Partial<CreateLeistungsbeurteilungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.LEISTUNGSBEURTEILUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'leistungsbeurteilungen') });
  }
  static async deleteLeistungsbeurteilungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.LEISTUNGSBEURTEILUNGEN}/records/${id}`);
  }
